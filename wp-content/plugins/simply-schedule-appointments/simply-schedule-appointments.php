...
final class Simply_Schedule_Appointments {
	...
	public static function _uninstall(){
		...
		if ( ! empty( $settings->developer->remove_data_on_uninstall) ) {
			...
			$wpdb->query("TRUNCATE TABLE wp_ssa_appointments");
			$wpdb->query("TRUNCATE TABLE wp_ssa_appointment_meta");
			$wpdb->query("TRUNCATE TABLE wp_ssa_appointment_types");
			$wpdb->query("TRUNCATE TABLE wp_ssa_availability");
			$wpdb->query("TRUNCATE TABLE wp_ssa_availability_external");
			$wpdb->query("TRUNCATE TABLE wp_ssa_async_actions");
			$wpdb->query("TRUNCATE TABLE wp_ssa_payments");
			$wpdb->query("TRUNCATE TABLE wp_ssa_staff");
			$wpdb->query("TRUNCATE TABLE wp_ssa_staff_appointments");
			$wpdb->query("TRUNCATE TABLE wp_ssa_staff_appointment_types");
		}
	}
	...
	public function meets_requirements() {
		if ( version_compare( phpversion(), '7.2', '<' ) ) {
			$this->activation_errors[] = 'Simply Schedule Appointments requires <strong>PHP version 7.2 or higher</strong>. Most WordPress hosts are supporting up to PHP 7, so your web host should easily be able to update you to PHP 7 if you contact them. <br />PHP 7+ is safer, faster, and best of all lets you use Simply Schedule Appointments :) <br /><a href="https://simplyscheduleappointments.com/">Learn More</a>';
			return false;
		}
	...
	public function get_current_edition() {
		...
		if ( ! in_array( $current_edition, array('1', '2', '3', '4') ) ) {
			return '4';
		}
